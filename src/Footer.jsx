...
import IconPools from "./assets/iconPools.png";

const Footer = () => {
  const location = useLocation();


    // if (location.pathname === "/sybilBlock") {
    //   return null;
    // }
  ...
  return (
    <footer className="BTNLow">
      <ul className="footerItems">
        ...
        <li className="footerItem">
          <Link className={`footerItemImgWrapper ${location.pathname !== "/pools" ? "img-dark" : ""}`} to="/pools"
            onClick={handleNavClick} >
            <img src={IconPools} alt="ONEX's" className="footerItemImg" />
          </Link>
          <p className={`footerItemLabel ${location.pathname !== "/pools" ? "footerItemLabel-dark" : ""}`}>
            POOLS
          </p>
        </li>
      </ul>
